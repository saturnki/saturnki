...
int ISOCDReadSectorFAD(u32 FAD, void *buffer) {
	...
	if (i == 100) {
		...
		printf("Warning: Sector not found");
		return 0;
	}
...
{
	return ISOCDGetStatus();
}

int32_t ReadToc(uint32_t* toc)
{
	return ISOCDReadTOC(toc);
}

int ReadSectorAtFad(uint32_t fad, void *buffer)
{
	return ISOCDReadSectorFAD(fad,buffer);
}

int CdInit(const char* filename)
{
